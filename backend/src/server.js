...
mongoose
  ...
  .then(() => {
    mongoConnected = true;
    const isLocal = process.env.MONGODB_URI.includes("127.0.0.1") || process.env.MONGODB_URI.includes("localhost");
    console.log(`✅ [DATABASE] Connected to ${isLocal ? "Local MongoDB" : "MongoDB Atlas"}`);
  })
  .catch((err) => {
    mongoError = err;
