...
const DepartmentCard = ({ department }) => {
  ...
  const departmentName = department?.name?.[language] || department?.name?.en || department?.name || t("unnamed_dept");
  const departmentDescription = department?.description?.[language] || department?.description?.en || t("dept_not_found");
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group">
      ...
      <div className="h-44 overflow-hidden relative">
        ...
        >
          {departmentBuilding === "A" ? `🏛️ ${t("building")} A` : `🏢 ${t("building")} B`}
        </div>
      </div>
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between text-sm text-gray-500 mb-3">
          <span className="flex items-center gap-1">
            <FaMapMarkerAlt className="text-emerald-500" /> {t("room")} {departmentRoom}
          </span>
          <span className="flex items-center gap-1">
