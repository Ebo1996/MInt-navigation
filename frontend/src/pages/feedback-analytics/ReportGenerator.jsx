...
export default function ReportGenerator() {
  ...
  const exportPDF = async () => {
    ...
    container.innerHTML = `
      <h1 style="color:#086976;margin-bottom:4px">MINT Navigator - Feedback Analytics Report</h1>
      <p style="color:#666;font-size:13px">Period: ${period.charAt(0).toUpperCase()+period.slice(1)} &nbsp;|&nbsp; ${startDate} to ${endDate}</p>
      ...
      <div style="display:flex;gap:16px;margin:20px 0;flex-wrap:wrap">
        <div style="background:#F0F4FA;padding:14px 20px;border-radius:8px;text-align:center;min-width:110px">
          <div style="color:#C8961E;font-size:24px;font-weight:900">${report.totalFeedback||0}</div>
          <div style="color:#666;font-size:12px;margin-top:4px">Total Feedback</div>
        </div>
        <div style="background:#F0F4FA;padding:14px 20px;border-radius:8px;text-align:center;min-width:110px">
          <div style="color:#C8961E;font-size:24px;font-weight:900">${(report.avgRating||0).toFixed(1)} ★</div>
          <div style="color:#666;font-size:12px;margin-top:4px">Avg Rating</div>
        </div>
        <div style="background:#F0F4FA;padding:14px 20px;border-radius:8px;text-align:center;min-width:110px">
          <div style="color:#C8961E;font-size:24px;font-weight:900">${report.responseRate||0}%</div>
          <div style="color:#666;font-size:12px;margin-top:4px">Response Rate</div>
        </div>
        <div style="background:#F0F4FA;padding:14px 20px;border-radius:8px;text-align:center;min-width:110px">
          <div style="color:#C8961E;font-size:24px;font-weight:900">${report.pendingFeedback||0}</div>
          <div style="color:#666;font-size:12px;margin-top:4px">Pending</div>
        </div>
      </div>
      ${rankings.length > 0 ? `
      <h2 style="color:#086976;margin-bottom:8px">Department Rankings</h2>
      <table style="width:100%;border-collapse:collapse;font-size:12px">
        <thead><tr>
          <th style="background:#086976;color:#fff;padding:8px;text-align:left">Rank</th>
          <th style="background:#086976;color:#fff;padding:8px;text-align:left">Department</th>
          <th style="background:#086976;color:#fff;padding:8px;text-align:left">Building</th>
          <th style="background:#086976;color:#fff;padding:8px;text-align:left">Avg Rating</th>
          <th style="background:#086976;color:#fff;padding:8px;text-align:left">Total Feedback</th>
          <th style="background:#086976;color:#fff;padding:8px;text-align:left">Response Rate</th>
        </tr></thead>
        <tbody>${rankings.map((d,i) => `<tr style="background:${i%2===0?"#fff":"#f9f9f9"}">
          <td style="border:1px solid #ddd;padding:6px">${i+1}</td>
          <td style="border:1px solid #ddd;padding:6px">${d?.name?.en||d?.name||""}</td>
          <td style="border:1px solid #ddd;padding:6px">${d?.building||""}</td>
          <td style="border:1px solid #ddd;padding:6px">${(d?.rating||0).toFixed(1)} ★</td>
          <td style="border:1px solid #ddd;padding:6px">${d?.totalFeedback||0}</td>
          <td style="border:1px solid #ddd;padding:6px">${d?.responseRate||0}%</td>
        </tr>`).join("")}</tbody>
      </table>` : ""}
    ...
    const opt = {
      margin:      [10,10,10,10],
      filename:    `MINT_Report_${period}_${startDate}_${endDate}.pdf`,
