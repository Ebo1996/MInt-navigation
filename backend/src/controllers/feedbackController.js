...
// @desc    Respond to feedback (sets main response field)
...
const respondToFeedback = async (req, res) => {
  try {
    ...
    feedback.respondedBy  = req.admin?._id  || null;
    feedback.respondedAt  = new Date();

    // Also push into the unified replies array so both dashboards see it
    feedback.replies.push({
      text:      response,
      role:      req.admin?.role || "feedback_analyst",
      name:      req.admin?.name || req.admin?.username || "Analyst",
      createdAt: new Date(),
    });

    await feedback.save();

    // Send email notification to visitor (non-blocking)
    let emailResult = { sent: false, reason: "missing_visitor_email" };
    if (feedback.visitorEmail && feedback.visitorEmail.trim()) {
      const senderName = req.admin?.name || req.admin?.username || "Ministry of Innovation & Technology";
      emailResult = await sendResponseEmail(feedback, response, senderName);
      if (!emailResult.sent) console.warn("Email not sent:", emailResult);
    }
...
// @desc    Add a threaded reply (analyst ↔ sector manager conversation)
...
const addReply = async (req, res) => {
  try {
    ...
    const reply = {
      text:      text.trim(),
      role:      req.admin?.role || "feedback_analyst",
      name:      req.admin?.name || req.admin?.username || "Admin",
      createdAt: new Date(),
    };

    feedback.replies.push(reply);

    // Set the top-level response field on first reply
    ...
    // Send email to visitor on every reply (non-blocking)
    let emailResult = { sent: false, reason: "missing_visitor_email" };
    if (feedback.visitorEmail && feedback.visitorEmail.trim()) {
      const senderName = req.admin?.name || req.admin?.username || "Ministry of Innovation & Technology";
      emailResult = await sendResponseEmail(feedback, text.trim(), senderName);
      if (!emailResult.sent) console.warn("Email not sent:", emailResult);
    }
