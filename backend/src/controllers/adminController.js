  ...
  try {
    ...
    if (admin) {
      ...
      res.json({
        ...
        departmentId: updatedAdmin.departmentId || null,
...
// ==============================================
// SECTOR MANAGER / DEPARTMENT HEAD MANAGEMENT
// ==============================================

// @desc    Get all department heads (and legacy sector managers)
// @route   GET /api/admin/sector-managers
const getSectorManagers = async (req, res) => {
  try {
    const managers = await Admin.find({ role: { $in: ["department_head", "sector_manager"] } })
      .select("-password")
      .sort({ sectorId: 1, departmentId: 1 });

    // Get additional stats for each manager
    const managersWithStats = await Promise.all(
      managers.map(async (manager) => {
        let departments;
        if (manager.role === "department_head" && manager.wing) {
          // Dept head sees only their wing's desks
          departments = await Department.find({ sectorId: manager.sectorId, wing: manager.wing });
        } else {
          departments = await Department.find({ sectorId: manager.sectorId });
        }
...
// @desc    Create department head
// @route   POST /api/admin/sector-managers
const createSectorManager = async (req, res) => {
  try {
    const { name, email, username, sectorId, wing, sendEmail } = req.body;
...
const updateSectorManager = async (req, res) => {
  try {
    const { name, email, sectorId, wing, newPassword } = req.body;
    const manager = await Admin.findById(req.params.id);
    if (!manager) return res.status(404).json({ message: "Department head not found" });

    if (name) manager.name = name;
    if (email) manager.email = email;
    if (sectorId) manager.sectorId = parseInt(sectorId);
    if (wing !== undefined) manager.wing = wing || null;
    if (newPassword) {
      const salt = await bcrypt.genSalt(10);
      manager.password = await bcrypt.hash(newPassword, salt);
    }
    await manager.save();
    res.json({ success: true, message: "Department head updated successfully" });
  } catch (error) {
