...
const getSectorFeedback = async (req, res) => {
  try {
    ...
    const isDeptHead = req.admin.role === "department_head";
    const userSectorId = req.admin.sectorId;
    const userWing     = req.admin.wing; // only set for department_head

    // Security: can only access their own sector
    if (parseInt(sectorId) !== userSectorId && req.admin.role !== "superadmin") {
      return res.status(403).json({ message: "Access denied" });
    }

    // Dept head sees only desks in their wing; sector manager sees all desks in sector
    let departments;
    if (isDeptHead && userWing) {
      departments = await Department.find({ sectorId: parseInt(sectorId), wing: userWing });
    } else {
      departments = await Department.find({ sectorId: parseInt(sectorId) });
    }
...
const getSectorStats = async (req, res) => {
  try {
    ...
    const isDeptHead = req.admin.role === "department_head";
    const userWing    = req.admin.wing;

    // Dept head sees only their wing's desks
    let departments;
    if (isDeptHead && userWing) {
      departments = await Department.find({ sectorId: parseInt(sectorId), wing: userWing });
    } else {
...
const respondToFeedback = async (req, res) => {
  try {
    ...
    // Access control: dept head → must own the wing; sector_manager → must own sector; superadmin → always
    if (req.admin.role === "department_head") {
      const department = await Department.findOne({ id: feedback.department });
      if (!department || department.wing !== req.admin.wing || department.sectorId !== req.admin.sectorId) {
        return res.status(403).json({ message: "Access denied: not your department" });
      }
    } else if (req.admin.role !== "superadmin") {
      const department = await Department.findOne({ id: feedback.department });
      if (!department || department.sectorId !== req.admin.sectorId) {
        return res.status(403).json({ message: "Access denied" });
      }
    }
    ...
    // Push into unified replies array
    feedback.replies.push({
      text:      response,
      role:      req.admin.role === "department_head" ? "department_head" : (req.admin.role || "sector_manager"),
      name:      req.admin.name || req.admin.username || "Department Head",
      createdAt: new Date(),
    });
    ...
    if (feedback.visitorEmail && feedback.visitorEmail.trim() !== "") {
      const senderName = req.admin.name || req.admin.username || "Department Head";
      emailResult = await sendResponseEmail(feedback, response, senderName);
      if (!emailResult.sent) console.warn("Email was not sent:", emailResult);
    }
...
const resolveFeedback = async (req, res) => {
  try {
    ...
    // Access control (same pattern as respond)
    if (req.admin.role === "department_head") {
      const department = await Department.findOne({ id: feedback.department });
      if (!department || department.wing !== req.admin.wing || department.sectorId !== req.admin.sectorId) {
        return res.status(403).json({ message: "Access denied: not your department" });
