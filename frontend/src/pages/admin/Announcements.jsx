...
const Announcements = () => {
  ...
  const filterAnnouncements = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (a) =>
          (a.title?.en || a.title || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
          (a.title?.am || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
          (a.message?.en || a.message || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
          (a.message?.am || "").toLowerCase().includes(searchTerm.toLowerCase()),
      );
    }
  ...
  const handleCreate = async () => {
    if (!formData.title.en.trim() || !formData.message.en.trim()) {
      toast.error("Please fill in Title and Message (English)");
      return;
    }
  ...
  const openEditModal = (announcement) => {
    ...
    setFormData({
      title: {
        en: announcement.title?.en || (typeof announcement.title === "string" ? announcement.title : "") || "",
        am: announcement.title?.am || "",
      },
      message: {
        en: announcement.message?.en || (typeof announcement.message === "string" ? announcement.message : "") || "",
        am: announcement.message?.am || "",
      },
                  ...
                  <div className="flex items-start justify-between flex-wrap gap-3">
                    <div className="flex-1">
                      ...
                      <h3 className="font-semibold text-gray-800 text-lg">
                        {announcement.title?.en || announcement.title}
                        {announcement.title?.am && (
                          <span className="block text-sm font-normal text-gray-500 mt-0.5" dir="auto">
                            {announcement.title.am}
                          </span>
                        )}
                      </h3>
                      <p className="text-gray-600 text-sm mt-1">
                        {announcement.message?.en || announcement.message}
                        {announcement.message?.am && (
                          <span className="block text-gray-400 mt-0.5" dir="auto">
                            {announcement.message.am}
                          </span>
                        )}
                      </p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, title: { ...formData.title, am: e.target.value } })
                      }
                      className="w-full px-3 py-2 border rounded-lg text-sm text-right"
                      placeholder="ለምሳሌ፣ የሚኒስቴሩ የዕረፍት ዕለት"
                      dir="auto"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, message: { ...formData.message, en: e.target.value } })
                      }
                      className="w-full px-3 py-2 border rounded-lg text-sm"
                  ...
                  <div>
                    ...
                    <textarea
                      rows="4"
                      value={formData.message.am}
                      onChange={(e) =>
                        setFormData({ ...formData, message: { ...formData.message, am: e.target.value } })
                      }
                      className="w-full px-3 py-2 border rounded-lg text-sm text-right"
                      placeholder="ዝርዝር የማስታወቂያ መልዕክት..."
                      dir="auto"
                    />
                  </div>
                </div>
