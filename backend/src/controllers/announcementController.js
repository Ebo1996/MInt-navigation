...
const createAnnouncement = async (req, res) => {
  try {
    ...
    // Support both { en, am } object and plain string (backward compat)
    const titleEn = cleanString(typeof title === "object" ? title?.en : title);
    const titleAm = cleanString(typeof title === "object" ? title?.am : "") || "";
    const messageEn = cleanString(typeof message === "object" ? message?.en : message);
    const messageAm = cleanString(typeof message === "object" ? message?.am : "") || "";

    if (!titleEn || !messageEn) {
      return res.status(400).json({ message: "Title (English) and Message (English) are required" });
    }
...
const updateAnnouncement = async (req, res) => {
  try {
    ...
    if (title !== undefined) {
      const enVal = cleanString(typeof title === "object" ? title?.en : title);
      if (!enVal) {
        return res.status(400).json({ message: "Title (English) cannot be empty" });
      }
      announcement.title = {
        en: enVal,
        am: cleanString(typeof title === "object" ? title?.am : "") || announcement.title?.am || "",
      };
    }

    if (message !== undefined) {
      const enVal = cleanString(typeof message === "object" ? message?.en : message);
      if (!enVal) {
        return res.status(400).json({ message: "Message (English) cannot be empty" });
      }
      announcement.message = {
        en: enVal,
        am: cleanString(typeof message === "object" ? message?.am : "") || announcement.message?.am || "",
      };
    }
