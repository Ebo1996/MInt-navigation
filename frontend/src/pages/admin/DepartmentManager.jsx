...
const DepartmentManager = () => {
  ...
  const loadSectors = async () => {
    try {
      ...
      setSectors(data.map(s => ({
        id: s.id,
        name: typeof s.name === "object" ? (s.name.en || s.name.am || "") : s.name,
      })));
    } catch {
      // fallback to 3 official sectors
      setSectors([
        { id: 1, name: "Central Administration & Governance Sector" },
        { id: 2, name: "Innovation & Research Sector" },
        { id: 3, name: "ICT & Digital Economy Sector" },
      ]);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this desk?")) {
      try {
        ...
        toast.error(error.message || "Failed to delete desk");
      }
    }
  };
  ...
  const handleDuplicate = async (dept) => {
    try {
      ...
      await loadDepartments();
      toast.success("Desk duplicated successfully");
    } catch (error) {
      toast.error(error.message || "Failed to duplicate desk");
    }
  };
  ...
  const handleAddDepartment = async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || error.message || "Failed to add desk",
      );
    } finally {
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `desks-export-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-emerald-100">
              Complete CRUD operations for all ministry desks
            </p>
          </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {paginatedDepts.map((dept) => (
                <tr key={dept.id} className="hover:bg-gray-50 transition">
                  ...
                  {/* Wing / Department column */}
                  <td className="px-4 py-3">
                    {editingId === dept.id ? (
                      <input
                        type="text"
                        value={editingData?.wing || ""}
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={newDepartment.name.am}
                      onChange={(e) =>
                        setNewDepartment({
                          ...newDepartment,
                          name: { ...newDepartment.name, am: e.target.value },
                        })
                      }
                      className="w-full px-3 py-2 border rounded-lg text-right"
                      placeholder="ለምሳሌ፣ የቴክኖሎጂ ማዕከል"
                      dir="auto"
                    />
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1 text-gray-700">
                    <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-emerald-600 text-white text-xs font-bold mr-2">1</span>
                    Select Sector *
                  </label>
                  <select
                    ...
                    onChange={(e) =>
                      setNewDepartment({
                        ...
                        wing: "", // reset department when sector changes
                      })
                    }
                ...
                {/* Step 2 — Department (filtered by selected sector) */}
                <div>
                  <label className="block text-sm font-semibold mb-1 text-gray-700">
                    <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-emerald-600 text-white text-xs font-bold mr-2">2</span>
                    Select Department *
                  </label>
                  <select
                    value={newDepartment.wing}
                    onChange={(e) =>
                      setNewDepartment({ ...newDepartment, wing: e.target.value })
                    }
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                  >
                    <option value="">-- Choose a department --</option>
                    {/* Show existing departments in selected sector */}
                    {[...new Set(
                      deptList
                        .filter(d => d.sectorId === newDepartment.sectorId && d.wing)
                        .map(d => d.wing)
                    )].map(w => (
                      <option key={w} value={w}>{w}</option>
                    ))}
                  </select>
                  {newDepartment.sectorId && [...new Set(
                    deptList.filter(d => d.sectorId === newDepartment.sectorId && d.wing).map(d => d.wing)
                  )].length === 0 && (
                    <p className="text-xs text-amber-600 mt-1">
                      No departments found for this sector. Type a new one below.
                    </p>
                  )}
                </div>

                {/* Allow typing a new department name if not in list */}
                {newDepartment.wing === "" && (
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="e.g., Minister's Office"
                      className="w-full px-3 py-2 border border-dashed rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none text-sm"
                      onChange={(e) =>
                        setNewDepartment({ ...newDepartment, wing: e.target.value })
                      }
                    />
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1 text-gray-700">
                    <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-emerald-600 text-white text-xs font-bold mr-2">3</span>
                    Building *
                  </label>
                  <select
                    value={newDepartment.building}
                    onChange={(e) =>
                      setNewDepartment({ ...newDepartment, building: e.target.value })
                    }
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                  >
                    <option value="A">Building A 🏛️</option>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setNewDepartment({
                          ...newDepartment,
                          description: { ...newDepartment.description, en: e.target.value },
                        })
                      }
                      className="w-full px-3 py-2 border rounded-lg"
                      placeholder="Describe the desk's responsibilities..."
                      dir="ltr"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      rows="3"
                      value={newDepartment.description.am}
                      onChange={(e) =>
                        setNewDepartment({
                          ...newDepartment,
                          description: { ...newDepartment.description, am: e.target.value },
                        })
                      }
                      className="w-full px-3 py-2 border rounded-lg text-right"
                      placeholder="የዴስኩን ኃላፊነቶች ይግለጹ..."
                      dir="auto"
                    />
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setNewDepartment({
                          ...newDepartment,
                          directions: { ...newDepartment.directions, en: e.target.value },
                        })
                      }
                      className="w-full px-3 py-2 border rounded-lg"
                  ...
                  <div>
                    ...
                    <textarea
                      rows="2"
                      value={newDepartment.directions.am}
                      onChange={(e) =>
                        setNewDepartment({
                          ...newDepartment,
                          directions: { ...newDepartment.directions, am: e.target.value },
                        })
                      }
                      className="w-full px-3 py-2 border rounded-lg text-right"
                      placeholder="ለምሳሌ፣ ሊፍት ወደ 2ኛ ፎቅ ይውሰዱ..."
                      dir="auto"
                    />
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={(newDepartment.services.am || []).join(", ")}
                      onChange={(e) =>
                        setNewDepartment({
                          ...newDepartment,
                          services: {
                            ...newDepartment.services,
                            am: e.target.value.split(",").map((s) => s.trim()).filter(Boolean),
                          },
                        })
                      }
                      className="w-full px-3 py-2 border rounded-lg text-right"
                      placeholder="ለምሳሌ፣ የቴክ ድጋፍ፣ የፈጠራ አስተዋፅኦ"
                      dir="auto"
                    />
                  </div>
                </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t">
                ...
                >
                  {imageUploading ? "Adding..." : "Add Desk"}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="bg-gradient-to-r from-emerald-600 to-teal-600 p-5 rounded-t-2xl flex justify-between items-center">
              <div>
                ...
                <p className="text-emerald-100 text-sm">Desk Details</p>
              </div>
              <button
              ...
              {selectedDept.head && (
                <div className="bg-gray-50 rounded-xl p-4 mb-4">
                  <h3 className="font-semibold mb-2 flex items-center gap-2">
                    <FiUser className="text-emerald-600" /> Desk Head
                  </h3>
